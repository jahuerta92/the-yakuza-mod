...
public class BatItem extends CustomItem {

    // Standard ID/Description
    public static final String ITEM_ID = YakuzaMod.makeID("BatItem");
    ...
    public BatItem(int upgraded, int durability) {
        super(ITEM_ID, orbString.NAME,
                durability,
                ATTACK_AMOUNT,
                SKILL_AMOUNT,
                THROW_AMOUNT,
                makeOrbPath("bat_item.png"));
    ...
    public void updateDescription() { // Set the on-hover description of the orb
        description = DESCRIPTIONS[0] + (2 * upgrades + attackValue) + DESCRIPTIONS[1] +
                DESCRIPTIONS[2] + (upgrades + skillValue) + DESCRIPTIONS[3] +
                DESCRIPTIONS[4] + upgrades * throwValue + DESCRIPTIONS[5];
    }

    @Override
    public int performAttackEffect(AbstractCard card) {
        applyCustomPowers(card);
        int currentAttackValue = 2 * (upgrades + attackValue);
        restoreValues();
    ...
    public void performSkillEffect(AbstractCard card) {
        AbstractDungeon.actionManager
                .addToBottom(
                        new GainBlockAction(AbstractDungeon.player, AbstractDungeon.player, (upgrades + skillValue)));
    }

    @Override
    public void performThrownEffect() {
        AbstractDungeon.actionManager.addToBottom(new DamageRandomEnemyAction(
                new DamageInfo(AbstractDungeon.player, upgrades * throwValue, DamageType.NORMAL),
