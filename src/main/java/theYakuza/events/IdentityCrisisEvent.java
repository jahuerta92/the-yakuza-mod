...
public class IdentityCrisisEvent extends AbstractImageEvent {
    ...
    public IdentityCrisisEvent() {
        ...
        if (AbstractDungeon.ascensionLevel >= 15) { // If the player is ascension 15 or above, lose 5% max hp. Else,
                                                    // lose just 3%.
            ...
            case 0: // While you are on screen number 0 (The starting screen)
                switch (i) {
                    case 0: // If you press button the first button (Button at index 0), in this case:
                        ...
                        this.imageEventText.updateDialogOption(0, OPTIONS[5]); // 1. Change the first button to the
                                                                               // [Leave] button
                        ...
                        AbstractRelic relicToAdd = RelicLibrary.starterList
                                .get(AbstractDungeon.relicRng.random(RelicLibrary.starterList.size() - 1)).makeCopy();
                        // Get a random starting relic

                        AbstractDungeon.getCurrRoom().spawnRelicAndObtain((float) (Settings.WIDTH / 2),
                                (float) (Settings.HEIGHT / 2), relicToAdd);

                        break; // Onto screen 1 we go.
                    case 1: // If you press button the second button (Button at index 1), in this case:
                            // Denial
                        ...
                        // (etc.)
                        // And that would also just set them into slot 0, 1, 2... in order, just like
                        // what we do in the very beginning

                        break; // Onto screen 1 we go.
                    case 2: // If you press button the third button (Button at index 2), in this case:
                            // Acceptance

                        AbstractCard c = new Apotheosis().makeCopy();
                        AbstractDungeon.effectList.add(new ShowCardAndObtainEffect(c, (float) (Settings.WIDTH / 2),
    ...
    public void update() { // We need the update() when we use grid screens (such as, in this case, the
                           // screen for selecting a card to remove)
        super.update(); // Do everything the original update()
        if (!AbstractDungeon.gridSelectScreen.selectedCards.isEmpty()) { // Once the grid screen isn't empty (we
                                                                         // selected a card for removal)
            AbstractCard c = (AbstractCard) AbstractDungeon.gridSelectScreen.selectedCards.get(0); // Get the card
            AbstractDungeon.topLevelEffects
                    .add(new PurgeCardEffect(c, (float) (Settings.WIDTH / 2), (float) (Settings.HEIGHT / 2))); // Create
                                                                                                               // the
                                                                                                               // card
                                                                                                               // removal
                                                                                                               // effect
