...
public class BottledPlaceholderRelic extends CustomRelic implements CustomBottleRelic, CustomSavable<Integer> {
    ...
    // Relic and the Custom Savable - they go hand in hand.
    ...
    // BasemodWiki Says: "When you need to store a value on a card or relic between
    // runs that isn't a relic's counter value
    // or a card's misc value, you use a custom savable to save and load it between
    // runs."

    private static AbstractCard card; // The field value we wish to save in this case is the card that's going to be
                                      // in our bottle.
    ...
    // Now, for making Bottled cards we need a small patch - our own custom
    // SpireField
    ...
    public void onEquip() { // 1. When we acquire the relic
        ...
        CardGroup group = CardGroup.getGroupWithoutBottledCards(AbstractDungeon.player.masterDeck); // 5. Get a card
                                                                                                    // group of all
                                                                                                    // currently
                                                                                                    // unbottled cards
        AbstractDungeon.gridSelectScreen.open(group, 1, DESCRIPTIONS[3] + name + DESCRIPTIONS[2], false, false, false,
                false);
        // 6. Open the grid selection screen with the cards from the CardGroup we
        // specified above. The description reads "Select a card to bottle for" + (relic
        // name) + "."
    }

    @Override
    public void onUnequip() { // 1. On unequip
        if (card != null) { // If the bottled card exists (prevents the game from crashing if we removed the
                            // bottled card from our deck for example.)
            AbstractCard cardInDeck = AbstractDungeon.player.masterDeck.getSpecificCard(card); // 2. Get the card
            if (cardInDeck != null) {
                BottledPlaceholderField.inBottledPlaceholderField.set(cardInDeck, false); // In our SpireField - set the
                                                                                          // card to no longer be
                                                                                          // bottled. (Unbottle it)
            }
        }
    }
    ...
    public void update() {
        ...
        if (!cardSelected && !AbstractDungeon.gridSelectScreen.selectedCards.isEmpty()) {
            // If the card hasn't been bottled yet and we have cards selected in the
            // gridSelectScreen (from onEquip)
            ...
            BottledPlaceholderField.inBottledPlaceholderField.set(card, true); // Use our custom spire field to set that
                                                                               // card to be bottled.
            if (AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.INCOMPLETE) {
                AbstractDungeon.getCurrRoom().phase = AbstractRoom.RoomPhase.COMPLETE;
            }
            AbstractDungeon.getCurrRoom().phase = AbstractRoom.RoomPhase.COMPLETE; // The room phase can now be set to
                                                                                   // complete (From INCOMPLETE in
                                                                                   // onEquip)
            AbstractDungeon.gridSelectScreen.selectedCards.clear(); // Always clear your grid screen after using it.
            setDescriptionAfterLoading(); // Set the description to reflect the bottled card (the method is at the
                                          // bottom of this file)
        }
    }
    ...
    public void onUseCard(AbstractCard targetCard, UseCardAction useCardAction) { // Whenever we use any card
        boolean fullHandDialog = false; // Create a boolean (to prevent multiple "My hand is full!" dialogues if we have
                                        // multiple cards bottled)

        for (Iterator<AbstractCard> it = AbstractDungeon.player.drawPile.group.iterator(); it.hasNext();) {
            // Create a new Iterator called "it" that checks for all AbstractCards in our
            // draw pile. For each card:
            AbstractCard card = it.next(); // create a new AbstractCard named "card" which is equal to the current card
                                           // in the for each loop
            if (BottledPlaceholderField.inBottledPlaceholderField.get(card)) { // Check if our SpireField matches said
                                                                               // card
                // Essentially, we end up with: Check if the draw pile has a card that is
                // bottled with this bottle
                ...
                if (AbstractDungeon.player.hand.size() < BaseMod.MAX_HAND_SIZE) { // If your hand isn't full
                    if (AutoplayField.autoplay.get(card)) { // If the card auto-plays - auto play it
                        AbstractDungeon.actionManager
                                .addToBottom(new AutoplayCardAction(card, AbstractDungeon.player.hand));
                    }
                    card.triggerWhenDrawn(); // If the card triggers an effect on being drawn - trigger it
                    AbstractDungeon.player.drawPile.moveToHand(card, AbstractDungeon.player.drawPile); // Move the card
                                                                                                       // to your hand
                                                                                                       // from your draw
                                                                                                       // pile

                    for (AbstractRelic r : AbstractDungeon.player.relics) { // And if you have any relics that trigger
                                                                            // on card draw - trigger them
                        r.onCardDraw(card);
                    }
                } else { // If your hand IS full - create a single "My hand is full!" dialogue and move
                         // the card to the discard pile instead
                    if (!fullHandDialog) {
