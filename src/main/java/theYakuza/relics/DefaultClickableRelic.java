...
public class DefaultClickableRelic extends CustomRelic implements ClickableRelic { // You must implement things you want
                                                                                   // to use from StSlib
    /*
    ...
    private boolean usedThisTurn = false; // You can also have a relic be only usable once per combat. Check out Hubris
                                          // for more examples, including other StSlib things.
    private boolean isPlayerTurn = false; // We should make sure the relic is only activateable during our turn, not the
                                          // enemies'.
    ...
    public void onRightClick() {// On right click
        if (!isObtained || usedThisTurn || !isPlayerTurn) {
            // If it has been used this turn, the player doesn't actually have the relic
            // (i.e. it's on display in the shop room), or it's the enemy's turn
            return; // Don't do anything.
        }

        if (AbstractDungeon.getCurrRoom() != null
                && AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT) { // Only if you're in combat
            usedThisTurn = true; // Set relic as "Used this turn"
            flash(); // Flash
            stopPulse(); // And stop the pulsing animation (which is started in atPreBattle() below)

            AbstractDungeon.actionManager.addToBottom(new TalkAction(true, DESCRIPTIONS[1], 4.0f, 2.0f)); // Player
                                                                                                          // speech
                                                                                                          // bubble
                                                                                                          // saying "YOU
                                                                                                          // ARE MINE!"
                                                                                                          // (See relic
                                                                                                          // strings)
            AbstractDungeon.actionManager.addToBottom(new SFXAction("MONSTER_COLLECTOR_DEBUFF")); // Sound Effect Action
                                                                                                  // of The Collector
                                                                                                  // Nails
            AbstractDungeon.actionManager.addToBottom(new VFXAction( // Visual Effect Action of the nails applies on a
                                                                     // random monster's position.
                    new CollectorCurseEffect(AbstractDungeon.getRandomMonster().hb.cX,
        ...
        // See that talk action? It has DESCRIPTIONS[1] instead of just hard-coding "You
        // are mine" inside.
    ...
    public void atTurnStart() {
        usedThisTurn = false; // Resets the used this turn. You can remove this to use a relic only once per
                              // combat rather than per turn.
