...
public class CommonPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public static final String POWER_ID = YakuzaMod.makeID("CommonPower");
    ...
    // On use card, apply (amount) of Dexterity. (Go to the actual power card for
    // the amount.)
    @Override
    ...
    // Note: If you want to apply an effect when a power is being applied you have 3
    // options:
    // onInitialApplication is "When THIS power is first applied for the very first
    // time only."
    // onApplyPower is "When the owner applies a power to something else (only used
    // by Sadistic Nature)."
    // onReceivePowerPower from StSlib is "When any (including this) power is
    ...
    public void atEndOfTurn(final boolean isPlayer) {
        ...
        for (final AbstractCard c : AbstractDungeon.actionManager.cardsPlayedThisTurn) {
            // This is how you iterate through arrays (like the one above) and card groups
            // like
            // "AbstractDungeon.player.masterDeck.getAttacks().group" - every attack in your
            // actual master deck.
            // Read up on java's enhanced for-each loops if you want to know more on how
            // these work.

            ++count; // At the end of your turn, increase the count by 1 for each card played this
                     // turn.
        }
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                // DO NOT HARDCODE YOUR STRINGS ANYWHERE: i.e. don't write any Strings directly
                // i.e. "Dexterity" for the power ID above.
                // Use the power/card/relic etc. and fetch it's ID like shown above. It's really
                // bad practice to have "Strings" in your code:
                 ...
                 * and if you ever want to change a string
